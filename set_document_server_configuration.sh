...
if ! grep -q "nginx" trusted_domain.tmp; then
    ...
    docker exec -u www-data nextcloud-fpm php occ --no-warnings config:system:set trusted_domains $TRUSTED_INDEX --value="nginx"
fi
...
docker exec -u www-data nextcloud-fpm php occ --no-warnings config:system:set onlyoffice StorageUrl --value="http://nginx/"
