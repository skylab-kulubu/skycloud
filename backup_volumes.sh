...
elif [[ $1 == "stirling-pdf"  ]];then
    docker run --rm --volumes-from stirling-pdf-trainingData -v $(pwd):/backup ubuntu tar cvf /backup/backup.tar /stirling-pdf-trainingData_backup
    docker run --rm --volumes-from stirling-pdf-extraConfig -v $(pwd):/backup ubuntu tar cvf /backup/backup.tar /stirling-pdf-extraConfig_backup
